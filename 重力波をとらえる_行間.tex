...
ここで式\ref{eq:metric}、\ref{eq:nmudown}式\ref{eq:nmuup}と直前の4式を用いて、外部曲率$K_{ij}$を計算すると
\begin{align}
...
と計算できる。この式は普通
\begin{align}
	\pdv{\gamma_{ij}}{t} = -2\alpha K_{ij} + \beta_{i|j} + \beta_{j|i}
\end{align}
と書くことが多いらしい。
...
さて、それぞれの式を$\alpha$、$\beta^i$、$\gamma_{ij}$で表現しようと頑張って計算すると次のようになる。
\begin{align}
